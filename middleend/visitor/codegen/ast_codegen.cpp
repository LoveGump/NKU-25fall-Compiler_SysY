        ...
        for (auto* vd : *(decls->decl->decls))
        {
            ...
            if (attr.arrayDims.empty())
            {
                ...
                if (!attr.initList.empty())
                {
                    ...
                    else if (elemType == DataType::F32) { initOp = getImmeF32Operand(attr.initList[0].getFloat()); }
                }
                // 没有初始值则为零初始化
            ...
            else
            {
                ...
                if (vd->init)
                {
                    ...
                    for (const auto& slot : initSlots)
                    {
                        ...
                        if (!exprVal.isConstexpr)
                        {
                            ERROR("Global array initializer must be constant expression at line %d, col %d",
                                initNode->line_num,
                                initNode->col_num);
                        }
