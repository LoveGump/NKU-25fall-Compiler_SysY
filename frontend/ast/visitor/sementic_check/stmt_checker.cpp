...
namespace FE::AST
{
    ...
    bool ASTChecker::visit(FuncDeclStmt& node)
    {
        ...
        if (node.params){
            ...
            {
                success = apply(*this, *param) && success;  
            }
        }
        ...
        if (node.body)
        {
            success = apply(*this, *node.body) && success;
        }

        if(success && this->curFuncRetType != voidType && !this->funcHasReturn)
        {
            // 非void函数缺少return语句
            this->errors.emplace_back("Function '" + node.entry->getName() + "' missing return statement at line " + std::to_string(node.line_num));
            success = false;
        }
        ...
        if (node.stmts){
            for (auto* stmt : *(node.stmts))
            {
                ...
                if(auto* funcDecl = dynamic_cast<FuncDeclStmt*>(stmt)){
                    this->errors.emplace_back("Function definition not allowed here at line " + std::to_string(funcDecl->line_num));
                    success = false;
                    continue;
                }
    ...
    bool ASTChecker::visit(ReturnStmt& node)
    {
        ...
        if(curFuncRetType == voidType){
            ...
            if(node.retExpr){
                this->errors.emplace_back("Return statement with a value in void function at line " + std::to_string(node.line_num));
                success = false;
            }
            return success;
        } 
        ...
        // 非void函数，必须有返回值且类型匹配
        if(!node.retExpr){
            this->errors.emplace_back("Return statement missing a value in non-void function at line " + std::to_string(node.line_num));
            return false;
        } 
        ...
        if(!exprType){
            // 无法确定返回值类型
            this->errors.emplace_back("Cannot determine return expression type at line " + std::to_string(node.line_num));
            return false;
        }
        // 匹配规则：允许隐式类型转换（如int到float），但不允许不兼容类型
        if(this->curFuncRetType->getTypeGroup() == TypeGroup::POINTER || exprType->getTypeGroup() == TypeGroup::POINTER){
            // 指针类型必须完全匹配
    ...
    bool ASTChecker::visit(IfStmt& node)
    {
        ...
        // 访问then分支
        if(node.thenStmt){
            success = apply(*this, *node.thenStmt) && success;
        }
        // 访问else分支
        if(node.elseStmt){
            success = apply(*this, *node.elseStmt) && success;
        }
        return success;
    }
    ...
    bool ASTChecker::visit(ForStmt& node)
    {
        ...
        // 访问初始化语句
        if(node.init){
            success = apply(*this, *node.init) && success;
        }
        // 访问条件表达式
        if(!node.cond){
        ...
        // 访问步进表达式
        if(node.step){
            success = apply(*this, *node.step) && success;
        }
        // 访问循环体
        if(node.body){
            success = apply(*this, *node.body) && success;
